...
export class InputRadio {
  ...
  private async getOptionElementList(): Promise<InputRadioOptionInfo[]> {
    ...
    for (let index = 0; index < count; index++)
      optionElementList.push(
        new InputRadioOptionInfo(
          new InputBase(await this._rootElementList.get(index))
        )
      );
  ...
  async getSelectedValue(): Promise<string> {
    const selectedValues = await filter$(
      ...
      async optionElement =>
        'true' ===
        (await (await optionElement.getInput())._element!.getAttribute(
          `checked`
        ))
    );
    ...
    if (selectedValues.length > 1)
      throw new Error(
        `More than one option (${selectedValues.length}) is selected`
      );
    else if (selectedValues.length === 0) return '';
    else
      return (await selectedValues[0].getInput())._element!.getAttribute(
        'value'
      ); // getValueString() rather
  }
  ...
  async setSelectedValue(target: string): Promise<void> {
    const matchingInputs = await filter$(
      ...
      async optionElement =>
        target ===
        (await (await optionElement.getInput())._element!.getAttribute('value'))
    );

    if (matchingInputs.length > 1)
      throw new Error(`More than one option with target value were found`);
    else if (matchingInputs.length === 0)
      throw new Error(`No options with target value were found`);
    else await (await matchingInputs[0].getInput())._element!.click(); // setValueString() rather
  }
}
